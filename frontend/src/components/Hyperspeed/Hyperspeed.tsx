...
export interface HyperspeedOptions {
  ...
  movingCloserSpeed: [-120, -160] | [number, number];
...
class CarLights {
  ...
  update(time: number) {
    if (this.mesh && this.mesh.material && this.mesh.material.uniforms.uTime) {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    if (!this.renderer.getContext()) {
      try {
        ...
        console.warn('Failed to initialize renderer context:', e);
      }
    }
  ...
  initPasses() {
    if (!this.renderer.getContext()) {
      try {
        ...
        console.warn('Failed to initialize WebGL context:', e);
      }
    }
    if (!this.renderer.getContext()) {
      return;
